...
class Canvas(QtWidgets.QLabel):
    ...
    def __init__(self):
        ...
        self.curves = []
        self.fillDensity = False

    def set_pen_color(self, c):
        self.pen_color = QtGui.QColor(c)

    def mouseMoveEvent(self, e):
        if self.mode in ["trajectory", "initial_density", "target_density"]:
            ...
            if self.mode == "target_density":
                i = e.x()//int(self.blocSize)
                j = self.gridResolution - 1 - e.y()//int(self.blocSize)
                self.drawCell(i,j,255, 255, 0)
                self.targetDensity[i+j*self.gridResolution] = 1
            self.update()
    ...
    def mouseReleaseEvent(self, e):
        if self.fillDensity:
            ...
            if self.mode == "target_density":
                i = e.x()//int(self.blocSize)
                j = self.gridResolution - 1 - e.y()//int(self.blocSize)
                self.flood_fill(self.targetDensity, i+1, j)
